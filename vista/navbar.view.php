...
<body>
    <?php
    $currentDir = dirname($_SERVER['PHP_SELF']); // Obtiene el directorio actual

    // Comprobar si estás en controlador.php para ajustar la ruta a la carpeta vista
    if (basename($currentDir) == 'controlador') {
        $vistaDir = '../vista'; // Subir un nivel hacia la raíz y luego a la carpeta vista
    } else {
        $vistaDir = $currentDir; // Mantener la ruta actual para otros archivos en la carpeta vista
    }
    ?>
    <?php if (isset($_SESSION['usuari'])): // Si el usuario está logueado ?>
